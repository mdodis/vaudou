...
#define VD_CALLBACK_INVOKE(callback, ...)                             \
    do                                                                \
    {                                                                 \
        if (callback.entry.func != 0)                                 \
        {                                                             \
            callback.entry.func(__VA_ARGS__, callback.entry.usrdata); \
        }                                                             \
    } while (0)

#define VD_CALLBACK_SET(callback, fptr, usrdataa)	\
    do { 											\
        callback.entry.func = fptr;                 \
        callback.entry.usrdata = usrdataa;			\
    } while(0)
